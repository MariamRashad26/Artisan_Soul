      ...
      <main className="max-w-7xl mx-auto px-4 md:px-10 pb-20">
          <div className="row g-5">
            ...
            <div className="col-12 col-lg-8 space-y-6">
                  ...
                  <div key={idx} className="glass-panel p-6 rounded-2xl border-gray-100 d-flex flex-column flex-md-row gap-6 shadow-premium transition group hover:shadow-2xl">
                    ...
                    <div className="flex-grow-1 d-flex flex-column justify-content-between">
                      <div className="d-flex justify-content-between align-items-start gap-4">
                        ...
                        <p className="text-dark font-serif fs-3 fw-black">Rs. {item.product.price}</p>
                      </div>
              ...
              {cartItems.length > 0 && (
                <div className="pt-8">
                  <h3 className="fs-4 fw-black font-serif tracking-tight text-dark mb-4 d-flex align-items-center gap-2">
                    <span className="material-symbols-outlined text-primary fs-3">auto_fix_high</span>
                    Enhance Your Experience
                  </h3>
                  <div className="bg-dark text-white rounded-2xl p-6 d-flex flex-column flex-md-row align-items-center justify-content-between gap-5 shadow-premium position-relative overflow-hidden group">
                    <div className="position-absolute align-items-center top-[-50%] right-[-10%] size-64 bg-primary blur-[80px] rounded-full opacity-20 group-hover:scale-110 transition duration-700"></div>
                    <div className="d-flex align-items-center gap-4 text-center text-md-start position-relative z-1">
                      <div className="bg-white/10 p-4 rounded-circle text-primary">
                        <span className="material-symbols-outlined fs-2">videocam</span>
                      </div>
                      <div>
                        <p className="fw-black text-white fs-5 font-serif tracking-tight mb-1">Request Manufacturing Video</p>
                        <p className="text-[10px] fw-black tracking-widest text-uppercase text-white/50 mb-0">Receive a personalized cinematic video of your shoes being crafted.</p>
                      </div>
                    </div>
                    <div className="d-flex align-items-center gap-4 position-relative z-1">
                      <span className="fs-4 fw-black text-white">Rs. 50</span>
                      <button 
                        onClick={() => setVideoAdded(!videoAdded)}
                        className={`${videoAdded ? 'bg-secondary' : 'bg-primary'} text-white border-0 px-6 py-3 rounded-pill fw-black text-xs uppercase tracking-widest shadow-lg hover:scale-105 transition-all`}
                      >
                        {videoAdded ? 'Added' : 'Add'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="glass-panel p-8 rounded-2xl border-gray-100 shadow-premium">
                  ...
                  <div className="d-flex flex-column gap-4 mb-6">
                    <div className="d-flex justify-content-between text-secondary">
                      ...
                      <span className="fw-black text-dark">Rs. {subtotal.toLocaleString()}</span>
                    </div>
                    {videoAdded && (
                      <div className="d-flex justify-content-between text-secondary">
                        <span className="text-xs fw-bold tracking-widest uppercase">Video Package</span>
                        <span className="fw-black text-dark">Rs. {videoCost}</span>
                      </div>
                    )}
                    <div className="d-flex justify-content-between text-secondary">
                    ...
                    <div className="d-flex justify-content-between text-secondary">
                      ...
                      <span className="fw-black text-dark">Rs. {tax.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                  <div className="d-flex justify-content-between border-top border-gray-100 pt-6 mb-8">
                    ...
                    <span className="fs-3 fw-black text-primary tracking-tight">Rs. {total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  <Link 
