...
const Checkout = () => {
  ...
  const handleContinueToReview = () => {
    if (!paymentData.cardNumber || !paymentData.expiryDate || !paymentData.cvv) {
      setFormError('Please fill in your payment details before continuing.');
      return;
    }
    setFormError('');
    setStep(3);
        ...
        <div className="d-none d-md-flex align-items-center gap-4">
          {[
            ...
            <React.Fragment key={s.id}>
              ...
              {i < 2 && <div className={`h-px w-8 ${step > s.id ? 'bg-primary' : 'bg-white/20'}`}></div>}
            </React.Fragment>
          ))}
        </div>
          ...
          <div className="col-lg-8 animate-in slide-in-from-left duration-700">
            {step === 1 && (
              <section className="space-y-8">
                ...
                <div className="pt-10">
                  <button onClick={handleContinueToPayment} className="w-100 md:w-auto px-10 py-4 bg-primary text-white fs-6 fw-black text-xs text-uppercase tracking-widest rounded-pill shadow-lg hover:shadow-xl hover:scale-105 transition-all d-flex align-items-center justify-content-center gap-2 border-0">
                    Continue to Payment
            ...
            {step === 2 && (
              <section className="space-y-8 animate-in slide-in-from-right duration-500">
                <div className="mb-10">
                  <h1 className="display-6 fw-black font-serif tracking-tight mb-3">Secure Payment</h1>
                  <p className="text-secondary opacity-60 fs-6">Every transaction is encrypted and protected.</p>
                  {formError && (
                    <div className="mt-4 p-4 bg-danger text-white rounded-xl shadow-premium fw-bold fs-6 d-flex align-items-center gap-3">
                      <span className="material-symbols-outlined rounded-full bg-white/20 p-1">error</span>
                      {formError}
                ...
                <div className="glass-panel p-6 rounded-xl border-primary-10 mb-8 d-flex align-items-center justify-content-between">
                  <div className="d-flex align-items-center gap-4">
                    <div className="size-12 bg-primary rounded-full d-flex align-items-center justify-content-center text-white shadow-lg">
                      <span className="material-symbols-outlined fs-4">lock</span>
                    </div>
                    <div>
                      <h4 className="fs-6 fw-bold mb-0">Encrypted checkout</h4>
                      <p className="text-xs text-secondary opacity-60 mb-0 tracking-wide">256-bit SSL Protection</p>
                    </div>
                  </div>
                  <div className="d-flex gap-2">
                    <div className="h-8 w-12 bg-white rounded border border-gray-200 d-flex align-items-center justify-content-center opacity-40">VISA</div>
                    <div className="h-8 w-12 bg-white rounded border border-gray-200 d-flex align-items-center justify-content-center opacity-40">AX</div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="row g-4">
                    <div className="col-12">
                      <label className="text-xs fw-black text-primary text-uppercase tracking-widest mb-2 d-block">Card Number</label>
                      <div className="position-relative">
                        <input name="cardNumber" value={paymentData.cardNumber} onChange={handlePaymentInputChange} className="w-100 rounded-lg border border-gray-200 bg-white p-4 fs-6 outline-none focus:border-primary transition" placeholder="0000 0000 0000 0000" type="text" />
                        <span className="position-absolute end-4 top-50 translate-middle-y material-symbols-outlined text-secondary opacity-30">credit_card</span>
                      </div>
                    </div>
                    <div className="col-md-6">
                      <label className="text-xs fw-black text-primary text-uppercase tracking-widest mb-2 d-block">Expiry Date</label>
                      <input name="expiryDate" value={paymentData.expiryDate} onChange={handlePaymentInputChange} className="w-100 rounded-lg border border-gray-200 bg-white p-4 fs-6 outline-none focus:border-primary transition" placeholder="MM / YY" type="text" />
                    </div>
                    <div className="col-md-6">
                      <label className="text-xs fw-black text-primary text-uppercase tracking-widest mb-2 d-block">CVV</label>
                      <input name="cvv" value={paymentData.cvv} onChange={handlePaymentInputChange} className="w-100 rounded-lg border border-gray-200 bg-white p-4 fs-6 outline-none focus:border-primary transition" placeholder="000" type="text" />
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-10 d-flex flex-column flex-md-row gap-4">
                  ...
                  <button onClick={handleContinueToReview} className="px-10 py-4 bg-primary text-white fs-6 fw-black text-xs text-uppercase tracking-widest rounded-pill shadow-lg hover:shadow-xl hover:scale-105 transition-all d-flex align-items-center justify-content-center gap-2 border-0">
                    Review Order
                    <span className="material-symbols-outlined fs-5 ms-2">fact_check</span>
                  </button>
                </div>
              </section>
            )}
            ...
            {step === 3 && (
              <section className="space-y-8 animate-in zoom-in duration-500">
                <div className="mb-10">
                  ...
                  <p className="text-secondary opacity-60 fs-6">Please verify your order details before confirming.</p>
                </div>
                ...
                <div className="row g-4 mb-10">
                  ...
                  <div className="col-md-6">
                    <div className="p-6 bg-gray-50 rounded-xl border border-gray-100 h-100">
                      ...
                      <div className="d-flex align-items-center gap-3">
                        <span className="material-symbols-outlined text-secondary fs-4">credit_card</span>
                        <p className="fs-6 text-dark opacity-80 mb-0 fw-bold">•••• •••• •••• {paymentData.cardNumber ? paymentData.cardNumber.slice(-4) : '4242'}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 bg-primary-5 rounded-xl border-primary-20">
                  <div className="d-flex align-items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-secondary opacity-70 mb-0">By confirming, you acknowledge that each bespoke piece is handcrafted specifically for you.</p>
                    </div>
                  </div>
                </div>

                <div className="pt-10 d-flex flex-column flex-md-row gap-4">
                  <button onClick={() => setStep(2)} className="px-10 py-4 bg-transparent text-dark border border-gray-200 hover:border-dark fs-6 fw-black text-xs text-uppercase tracking-widest rounded-pill transition-all d-flex align-items-center justify-content-center gap-2">
                    Back
                  </button>
              ...
              <div className="space-y-6">
                  ...
                  {cartItems.map((item, idx) => (
                    <div key={idx} className="d-flex gap-4">
                      ...
                      <div className="py-1">
                        ...
                        <span className="fs-6 fw-black text-primary">Rs. {item.product.price} <span className="opacity-50 text-[10px]">x{item.quantity}</span></span>
                      </div>
                    </div>
                  ))}
                  {cartItems.length === 0 && (
                ...
                <div className="space-y-4 mt-6">
                  <div className="d-flex justify-content-between">
                    ...
                    <span className="fs-6 fw-bold text-dark">Rs. {getSubtotal().toLocaleString()}</span>
                  </div>
                  <div className="d-flex justify-content-between">
                  ...
                  <div className="d-flex justify-content-between">
                    ...
                    <span className="fs-6 fw-bold text-dark">Rs. {(getSubtotal() * 0.08).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  <div className="pt-6 border-top border-gray-100 d-flex justify-content-between align-items-baseline">
                    ...
                    <span className="fs-3 fw-black text-primary tracking-tight">Rs. {(getSubtotal() * 1.08).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
