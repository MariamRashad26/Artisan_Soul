...
const AdminOrders = () => {
  ...
  const filterConfig = [
    { key: 'stage', label: 'Pipeline Stage', options: ['All Stages', 'Cutting', 'Lasting', 'Stitching', 'Finished'] },
    { key: 'type', label: 'Entry Type', options: ['All Types', 'Bespoke', 'Stock'] },
    { key: 'priority', label: 'Priority', options: ['All Priorities', 'Regular', 'VIP Expedited', 'Legacy Archive'] }
  ];
  ...
  const handleManualEntry = async (e) => {
    ...
    try {
      const { data } = await axios.post('/api/orders', newBackendOrder);
      const mapped = {
          id: data.orderId,
          patron: data.patron,
          model: data.model,
  ...
  const handleExecute = async (id) => {
    ...
    try {
      await axios.put(`/api/orders/${encodeURIComponent(id)}`, {
  ...
  return (
    <div className="p-8 lg:p-12 animate-in fade-in duration-1000">
        ...
        <section className="bg-white p-6 rounded-[2rem] border border-stone-100 shadow-sm d-flex flex-wrap gap-6 mb-10 animate-in slide-in-from-top-4 duration-500">
          ...
          <div className="d-flex flex-column gap-2 min-w-[200px]">
             ...
             <select value={complexFilters.artisan} onChange={e => setComplexFilters({...complexFilters, artisan: e.target.value})} className="bg-stone-50/50 border border-stone-100 rounded-xl px-4 py-2 text-xs fw-medium text-dark outline-none focus:border-dark">
               ...
               <option value="Lorenzo Giamatti">Lorenzo Giamatti</option>
               <option value="Marco Sartori">Marco Sartori</option>
             </select>
          </div>
                ...
                <tr key={i} className="group hover:bg-stone-50/50 transition-colors duration-500 cursor-pointer">
                  ...
                  <td className="px-10 py-8 text-end">
                    <div className="d-flex justify-content-end gap-3 transition-all duration-500">
                      ...
                      <button onClick={() => handleExecute(row.id)} className="px-5 py-2 rounded-xl bg-dark text-white text-[10px] fw-black text-uppercase tracking-widest hover:bg-stone-800 transition duration-500 shadow-lg">
                        Advance
                      </button>
                    </div>
                  </td>
      ...
      {/* Holistic Operational Metrics */}
      <section className="row g-8 mt-12">
        {[
          { icon: 'precision_manufacturing', label: 'Active Builds', value: '42', unit: 'Pairs', detail: '+12% Throughput', trend: 'up' },
          { icon: 'history_edu', label: 'Master Certification', value: '14.2', unit: 'Days Avg.', detail: '0.5d optimization', trend: 'up' },
          { icon: 'account_balance_wallet', label: 'Order Valuation', value: '$84,200', unit: 'MTD', detail: '+8% Efficiency', trend: 'up' }
        ].map((metric, i) => (
