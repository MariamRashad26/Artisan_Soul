...
const ProductDetail = () => {
  ...
  return (
    <div className="flex-grow-1 bg-white animate-in fade-in duration-700">
          ...
          <div className="col-12 col-lg-5 d-flex flex-column gap-6">
            <div>
              ...
              <div className="d-flex align-items-baseline gap-3">
                <span className="fs-3 fw-bold text-primary tracking-tight">PKR {product.price}</span>
                {product.discount && (
                  <span className="text-secondary opacity-40 text-decoration-line-through">PKR {product.price + product.discount}</span>
                )}
              </div>
            </div>
            ...
            <div className="pt-6 border-top border-gray-100">
              <div className="mb-6">
                <div className="d-flex justify-content-between align-items-center mb-4">
                  ...
                  <button onClick={() => setShowSizeGuide(true)} className="btn btn-link p-0 text-primary text-decoration-none fw-bold text-xs opacity-80 border-0 bg-transparent">SIZE GUIDE</button>
                </div>
      ...
      {/* Size Guide Modal */}
      {showSizeGuide && (
        <div className="position-fixed top-0 start-0 w-100 h-100 z-50 d-flex align-items-center justify-content-center p-4" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)' }}>
          <div className="bg-white rounded-3xl p-8 max-w-lg w-100 shadow-2xl animate-in zoom-in-95 duration-200" style={{ maxHeight: '85vh', overflowY: 'auto' }}>
            <div className="d-flex justify-content-between align-items-center mb-6">
              <h3 className="fs-4 fw-black font-serif mb-0">Size Guide</h3>
              <button onClick={() => setShowSizeGuide(false)} className="border-0 bg-transparent text-secondary hover:text-dark transition">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <p className="text-secondary text-sm mb-6">Artisan Soul shoes are crafted on European lasts. If you are between sizes, we recommend sizing up.</p>
            <table className="table table-sm table-bordered text-center">
              <thead className="table-dark">
                <tr>
                  <th>EU</th><th>UK</th><th>US</th><th>Foot Length (cm)</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={eu} className={selectedSize === eu ? 'table-warning fw-bold' : ''}>
                    <td>{eu}</td><td>{uk}</td><td>{us}</td><td>{cm}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <button onClick={() => setShowSizeGuide(false)} className="btn btn-primary w-100 rounded-pill mt-4 fw-bold">Got It</button>
          </div>
        </div>
      )}
    </div>
