    ...
    <div className="d-flex vh-100 overflow-hidden bg-background-light font-display text-dark">
      ...
      <aside className="w-64 flex-shrink-0 border-end border-light-subtle border-primary/10 bg-white d-none d-md-flex flex-column">
        <div className="p-4 d-flex flex-column h-100">
          ...
          <div className="mt-auto p-3 bg-primary/5 rounded-4 border border-light-subtle border-primary/10">
            <div className="d-flex align-items-center gap-3">
              <div className="h-10 w-10 rounded-circle bg-stone-850 d-flex align-items-center justify-content-center text-dark border border-primary/20 shadow-sm text-uppercase fw-bold text-[12px]">
                {user?.name ? user.name.substring(0, 2) : 'US'}
              </div>
              <div>
                <p className="fs-6 text-muted fw-bold text-dark mb-0">{user?.name || 'Client'}</p>
      ...
      <main className="flex-grow-1 overflow-y-auto w-100">
        ...
        <div className="max-w-6xl mx-auto p-4">
          ...
          <section className="mb-8">
            ...
            {/* Current Clip Info */}
            {currentClip && (
              <div className="bg-white p-5 rounded-3xl mt-4 border border-light-subtle">
                <div className="d-flex justify-content-between align-items-start">
                  <div>
                    <h2 className="fs-3 fw-bold tracking-tight mb-2">{currentClip.title}</h2>
                    <p className="text-stone-600">{currentClip.tag} • {currentClip.duration}</p>
                  </div>
                  <span className="badge bg-success text-white px-3 py-2">LIVE NOW</span>
                </div>
              </div>
            )}
          </section>
                    ...
                    <div className={`rounded-3xl overflow-hidden cursor-pointer border-2 transition-all hover:shadow-xl ${currentClip?._id === clip._id ? 'border-primary shadow-xl' : 'border-transparent'}`}>
                      <div className="aspect-video relative">
                        <img 
                          ...
                          alt={clip.title} 
                        />
                        <div className="position-absolute top-3 right-3 bg-black/70 text-white text-[10px] px-2 py-1 rounded tracking-widest">
                          LIVE
                        </div>
                      </div>
                      <div className="p-4 bg-white">
                        <p className="fw-bold text-sm mb-1 line-clamp-2">{clip.title}</p>
                        <p className="text-xs text-stone-500">{clip.tag}</p>
                      </div>
                    </div>
                  </div>
                ))}
