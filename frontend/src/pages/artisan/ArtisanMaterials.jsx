...
const ArtisanMaterials = () => {
    ...
    try {
      ...
      if (logRes.data && Array.isArray(logRes.data)) {
        setLogs(logRes.data.map(l => ({
          id: l._id ? l._id.slice(-6).toUpperCase() : 'LOG-UNK',
          material: l.raw_material_id?.name || 'Unknown',
          batch: l.batch_id?.orderId || 'Unknown',
          amount: l.quantity_used || 0,
          unit: l.raw_material_id?.unit || '',
          date: new Date(l.date || l.createdAt || Date.now()).toLocaleDateString()
        })));
      }

      setMaterials(matRes.data || []);

      // Only show artisan's assigned orders in the dropdown
      ...
      const myOrders = (orderRes.data || []).filter(o => {
        if (!o) return false;
        const oId = (o._id || '').toString();
        const oArtisanId = (o.artisan_id || '').toString();
        const oArtisanName = (o.artisan || '').toLowerCase();
        return (
          (oId && assignedOrderIds.includes(oId)) ||
          (currentUserId && oArtisanId === currentUserId) ||
          (currentUserName && oArtisanName === currentUserName)
        );
      });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!user?._id) { alert('You must be logged in to log material usage.'); return; }
    try {
      ...
      console.error(err);
      alert('Failed to log material usage. Check that the order and material are selected.');
    }
  };
