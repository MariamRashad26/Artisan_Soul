  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // PRIMARY FILTER: Work orders assigned to this artisan via WorkOrder collection
        const assignedOrderIds = woRes.data
          .filter(wo => {
            const woAssignedId = (wo.assigned_to?._id || wo.assigned_to || '').toString();
            const woAssignedName = (wo.assigned_to?.name || '').trim().toLowerCase();
            return (
              (woAssignedId && currentUserId && woAssignedId === currentUserId) ||
              (woAssignedName && currentUserName && woAssignedName === currentUserName)
            );
          })
          .map(wo => (wo.order_id?._id || wo.order_id || '').toString());
        ...
        const activeOrders = ordersRes.data
          .filter(order => {
            ...
            // FALLBACK: also match orders where artisan field or artisan_id was set directly
            const orderArtisanName = (order.artisan || '').trim().toLowerCase();
            const orderArtisanId = (order.artisan_id || '').toString();

            return (
              assignedOrderIds.includes(orderIdStr) ||
              (orderArtisanId && currentUserId && orderArtisanId === currentUserId) ||
              (orderArtisanName && currentUserName && orderArtisanName === currentUserName)
            );
          })
          .map(order => ({
            id: order.orderId || '#AS-0000',
            _id: order._id,
      ...
      <header className="bg-white/80 backdrop-blur-xl border-bottom border-stone-200 px-6 lg:px-12 py-6 sticky-top z-40 shadow-sm animate-in slide-in-from-top duration-700">
        <div className="max-w-7xl mx-auto d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-6">
          ...
          <div className="d-flex gap-3">
            <button onClick={() => window.location.reload()} className="px-6 py-3.5 rounded-2xl border border-stone-200 text-stone-600 fw-black text-xs text-uppercase tracking-widest hover:bg-stone-50 transition duration-500 shadow-sm d-flex align-items-center gap-3">
              <span className="material-symbols-outlined fs-5 animate-spin-slow">sync</span>
              Sync Workshop
            </button>
            <Link to="/artisan" className="px-8 py-3.5 rounded-2xl bg-dark text-white fw-black text-xs text-uppercase tracking-widest shadow-xl hover:-translate-y-1 transition duration-500 d-flex align-items-center gap-3 text-decoration-none">
              Overview
        ...
        <section className="glass-panel rounded-[3rem] border-stone-100 shadow-premium overflow-hidden mb-10">
                ...
                {loading ? (
                  <tr>
                    <td colSpan="6" className="px-8 py-12 text-center">
                      <div className="d-flex flex-column align-items-center gap-4 text-stone-400">
                        ...
                        <span className="text-sm fw-bold">Loading your assignments...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr key={i} className="group hover:bg-stone-50 transition-colors duration-500 cursor-pointer">
                    ...
                    <td className="px-8 py-8 text-end">
                      <Link
                        to={`/artisan/production/${encodeURIComponent(order.id.replace('#', ''))}`}
                        className="size-10 bg-white border border-stone-100 rounded-xl d-flex align-items-center justify-content-center text-stone-400 hover:text-primary hover:border-primary hover:shadow-xl hover:-translate-x-1 transition-all duration-500"
                      >
                ...
                {!loading && allOrders.length === 0 && (
                  <tr>
                    <td colSpan="6" className="px-8 py-16 text-center">
                      <div className="d-flex flex-column align-items-center gap-4 text-stone-400">
                        <div className="size-20 bg-stone-50 rounded-full d-flex align-items-center justify-content-center">
                          <span className="material-symbols-outlined fs-1 text-stone-300">inbox</span>
                        </div>
                        <div>
                          <p className="fw-black text-stone-500 mb-1">No assignments yet</p>
                          <p className="text-xs text-stone-400 mb-0">Ask an admin to assign you to an order via the Admin → Orders panel.</p>
                        </div>
                      </div>
                    </td>
                  </tr>
          ...
          {totalPages > 1 && (
            <div className="px-8 py-6 bg-stone-50/30 d-flex justify-content-between align-items-center border-top border-stone-50">
              <p className="text-[11px] fw-black text-stone-400 text-uppercase tracking-widest mb-0">
                Page {currentPage < 10 ? `0${currentPage}` : currentPage} of {totalPages < 10 ? `0${totalPages}` : totalPages}
                <span className="mx-3 opacity-20">|</span> {allOrders.length} Assignment Records
              </p>
        ...
        <section className="bg-stone-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group">
          ...
          <div className="row g-8 align-items-center relative z-1">
            ...
            <div className="col-md-10">
              ...
              <p className="text-white/40 fs-6 fw-medium leading-relaxed mb-0">
                Excellent pace, {user?.name || 'Artisan'}. Complete your assigned orders and submit quality reports after each stage.
              </p>
            </div>
          </div>
        </section>
