    ...
    <div className="d-flex flex-column flex-md-row vh-100 overflow-hidden bg-background-light font-display text-dark">
          ...
          <aside className="position-absolute left-0 top-0 h-100 w-80 bg-dark d-flex flex-column shadow-2xl animate-in slide-in-from-left duration-300">
            <div className="p-6 d-flex align-items-center justify-content-between border-bottom border-white/10">
              <Link to="/" className="d-flex align-items-center gap-4 group text-decoration-none" onClick={() => setIsMobileSidebarOpen(false)}>
                <div className="size-10 rounded-xl bg-primary d-flex align-items-center justify-content-center text-white">
                  <span className="material-symbols-outlined fs-4">shopping</span>
                </div>
                <div>
                  <h1 className="font-serif fs-5 fw-black text-white leading-none tracking-tighter mb-0 lowercase">Artisan.Soul</h1>
                  <span className="text-[9px] fw-black text-uppercase tracking-[0.2em] text-primary-20">Private Atelier</span>
                </div>
              </Link>
              <button 
                onClick={() => setIsMobileSidebarOpen(false)} 
            ...
            <div className="p-6 border-top border-white/10 bg-black/20">
              <div className="p-4 rounded-3xl border border-white/10 bg-white/5 d-flex align-items-center justify-content-between">
                <div className="d-flex align-items-center gap-4">
                  <div className="size-10 rounded-xl bg-stone-800 d-flex align-items-center justify-content-center text-white fw-bold border-2 border-primary/50 shadow-sm text-uppercase">
                    {user?.name ? user.name.substring(0, 2) : 'US'}
                  </div>
                  <div className="d-flex flex-column">
                    <span className="text-xs fw-black text-white tracking-tight">{user?.name || 'Client'}</span>
      ...
      <main className="flex-grow-1 overflow-y-auto w-100 bg-background-light ">
        ...
        <header className="sticky-top top-0 z-40 bg-white border-bottom border-stone-200 px-6 py-3 h-16 d-flex d-md-none align-items-center justify-content-between shadow-sm">
          ...
          <span className="text-[10px] fw-black text-uppercase tracking-wider text-primary px-3 py-1 bg-primary/10 rounded-full">Atelier</span>
        </header>
